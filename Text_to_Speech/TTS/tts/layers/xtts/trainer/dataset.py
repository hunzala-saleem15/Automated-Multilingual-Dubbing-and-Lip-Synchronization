...
class XTTSDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        MAX_TRIES = 50  # prevent infinite recursion
        tries = 0
        while tries < MAX_TRIES:
            ...
            try:
                ...
                print(f"Exception: {e}")
                raise e  # stop here so you can see why

            if wav is None or (self.max_wav_len and wav.shape[-1] > self.max_wav_len) or \
            (self.max_text_len and tseq.shape[0] > self.max_text_len):
                self.failed_samples.add(sample_id)
                continue

            if isinstance(cond_len, float) and math.isnan(cond_len):
                cond_len_tensor = torch.tensor(0, dtype=torch.long)  # or None
            else:
                cond_len_tensor = torch.tensor(cond_len, dtype=torch.long)

            # safe conversion for cond_idxs
            if isinstance(cond_idxs, float) and torch.isnan(torch.tensor(cond_idxs)):
                cond_idxs_tensor = torch.tensor([0, 0], dtype=torch.long)  # or None
            else:
                cond_idxs_tensor = torch.tensor(cond_idxs, dtype=torch.long)
            ...
            return {
                ...
                "cond_lens": cond_len_tensor,
                "cond_idxs": cond_idxs_tensor,
            }

        # if all attempts fail
        raise RuntimeError("No valid sample could be loaded from the dataset.")
    ...
    def collate_fn(self, batch):
        ...
        return batch
    def __len__(self):
        # If eval mode, use self.samples as a list
        if self.is_eval:
            return len(self.samples)
        # If training, self.samples is a dict of languages
