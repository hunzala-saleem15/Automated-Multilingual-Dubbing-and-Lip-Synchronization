...
class Talklipdata(object):
    ...
    def croppatch(self, images, bbxs):
        patch = np.zeros((images.shape[0], self.crop_size, self.crop_size, 3), dtype=np.float32)
        ...
        for i, bbx in enumerate(bbxs):
            ...
            if bbx.size < 4:
                bbx = np.pad(bbx, (0, 4 - bbx.size), mode='constant')
            x0, y0, x1, y1 = [float(v) for v in bbx[:4]]
            ...
            if i < 5:
                print(f"Frame {i} bbox: {x0}, {y0}, {x1}, {y1}")

            patch[i] = cv2.resize(images[i, y0:y1, x0:x1, :], (self.crop_size, self.crop_size))
    ...
    def audio_visual_align(self, audio_feats, video_feats):
        ...
        if diff < 0:
            if isinstance(audio_feats, torch.Tensor):
                ...
                audio_feats = torch.cat([audio_feats, padding], dim=0)
            else:
                padding = np.zeros(
                    (-diff, audio_feats.shape[-1]),
                    dtype=audio_feats.dtype,
                )
                audio_feats = np.concatenate([audio_feats, padding], axis=0)

        elif diff > 0:
            left = diff // 2
            right = diff - left
            if right == 0:
                audio_feats = audio_feats[left:]
            else:
...
def model_synt(test_data_loader, device, model, args):
    ...
    for inps, spectrogram, idAudio, gt, padding_mask, pickedimg, imgs, bbxs, names in tqdm(test_data_loader):
        ...
        cv2.imwrite(
            "debug_frame0.png",
            processed_img[0][0].cpu().numpy().astype(np.uint8)
        )
        print("Saved debug_frame0.png")

        print("Processed videos:", len(processed_img))

        for i, video in enumerate(processed_img):
            out_path = '{}/{}.mp4'.format(args.save_root, names[i])
            os.makedirs(os.path.dirname(out_path), exist_ok=True)

            # Get actual frame dimensions (Height, Width) from the first frame
            sample_frame = video[0].cpu().detach().numpy().astype(np.uint8)
            height, width = sample_frame.shape[0], sample_frame.shape[1]

            print(f"Video {i}: {len(video)} frames | Dimensions: Width={width}, Height={height}")

            # OpenCV expects tuple as (width, height)
            h, w = video[0].shape[:2]

            print("Writer size:", (w, h))
            print("tmpvideo =", tmpvideo)
            ...
            count = 0
            for im in video:
                im = im.cpu().detach().numpy().astype(np.uint8)
                
                # Dynamic check: Ensure frame strictly matches VideoWriter dimensions
                if (im.shape[1], im.shape[0]) != (w, h):
                    im = cv2.resize(im, (w,h))

                if count == 0:
                    print("After Check - Frame shape:", im.shape)
                    print("Frame dtype:", im.dtype)
                    print("Frame min/max:", im.min(), im.max())
            ...
            print("Exists:", os.path.exists(tmpvideo))

            if os.path.exists(tmpvideo):
                print("Final Size:", os.path.getsize(tmpvideo))

            audio = '{}/{}.wav'.format(args.audio_root, names[i])

            command = '{} -y -i {} -i {} -strict -2 -q:v 1 {} -loglevel quiet'.format(
                args.ffmpeg, audio, tmpvideo, out_path
            )
            ...
            # AVI sirf tab delete karo jab MP4 successfully ban jaye
            if ret == 0 and os.path.exists(out_path):
                os.remove(tmpvideo)
            else:
                print("FFmpeg failed. Keeping AVI for debugging.")
...
if __name__ == "__main__":
    ...
    test_loader = DataLoader(test_dataset, batch_size=1, collate_fn=collate_fn, num_workers=0) #hparams.batch_size, 4,
