...
class TalkLip_disc_qual(nn.Module):
    ...
    def perceptual_forward(self, false_face_sequences):
        ...
        Returns:
            false_pred_loss
        """
        # AMP ko is block ke liye disable kar rahe hain taaki BCE crash na kare
        with torch.cuda.amp.autocast(enabled=False):
            # Input ko explicitly Float32 mein convert karein
            false_face_sequences = false_face_sequences.float()
            ...
            # Prediction aur Target dono Float32 mein honi chahiye
            pred = self.binary_pred(false_feats).view(len(false_feats), -1)
            target = torch.ones((len(false_feats), 1), device=false_face_sequences.device)
