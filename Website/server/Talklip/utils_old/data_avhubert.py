...
def emb_roi2im(pickedimg, imgs, bbxs, predictions, device):
    ...
    for b in range(len(imgs)):
        ...
        for idx in range(len(video_frames)):
            frame = video_frames[idx].numpy() if isinstance(video_frames[idx], torch.Tensor) else video_frames[idx]
            frame = frame.copy() # Ensure array is writable
            ...
            if bbx is None or np.all(bbx == 0):
            ...
            # Fetch prediction for current frame
            pred_patch = predictions[pred_idx].cpu().detach().numpy()
            
            # Convert (C, H, W) to (H, W, C) if required
            if pred_patch.shape[0] == 3:
            ...
            # 1. Resize TalkLip patch (96x96) to BBox dimensions (112x167)
            resized_pred = cv2.resize(pred_patch, (target_w, target_h))

            # 2. Create smooth elliptical alpha mask
            ...
            # 3. Dynamic Gaussian Blur for edge feathering
            ...
            original_crop = frame[y0:y1, x0:x1].astype(np.float32)
